...
while 803A:   not  $COUNTER1_TONI1 == $VANS_AMOUNT_TONI1
	...
	if and
		0214:   pickup $FREE_GREANDES picked_up 
		0038:   $PICKED_UP_GRENADES == 0 
	then
		0001: wait 1500 ms 
		03E5: text_box 'GREN_1'  // The longer you hold the ~h~~k~~PED_FIREWEAPON~ button~w~, the further you will throw the grenade.
		0004: $PICKED_UP_GRENADES = 1
	end
end //while
...
then
	if and
		...
		00DB:   is_char_in_car $TEMP_VANMAN_TONI1 car $TEMP_VAN_TONI1
	then
		00AD: set_car_cruise_speed $TEMP_VAN_TONI1 to 18.0 
		00AE: set_car_driving_style $TEMP_VAN_TONI1 to DRIVINGMODE_AVOIDCARS
